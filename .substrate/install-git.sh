...
prefix="${SUBSTRATE_GIT_PREFIX:-/opt/substrate-git}"

git_recent() {
    "${1:-git}" version 2>/dev/null \
        | grep -Eq 'git version 2\.(4[1-9]|[5-9][0-9])|git version [3-9]\.'
}

link_prefix() {
    local dest=/usr/local/bin/git
    [ -x "$prefix/bin/git" ] || return 1
    git_recent "$prefix/bin/git" || return 1
    ln -sf "$prefix/bin/git" "$dest"
    hash -r
    printf 'install-git: using %s (%s)\n' "$prefix/bin/git" "$("$prefix/bin/git" version)"
}
