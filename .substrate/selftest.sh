...
# the sandbox has no .git, so the history scan cannot run — disable it
# visibly rather than letting it fail for the wrong reason
...
note "sandbox: 50-gitleaks.sh disabled (no .git in sandbox)"
