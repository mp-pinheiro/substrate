...
Key oracles (graduate to child plan): stop-path jq count ≤ 3 under the A/B scenario matrix → `bash test/ab-stop-test.sh`; content-hash memo regression → `bash test/changed-scan-test.sh`; golden vectors verify → `bash test/golden-vectors-test.sh`; gate + both duplication-tax checks green → `substrate gate`.
Workflow: single agent, inline; enemy review on the jq batching (behavior-preserving refactor of fail-closed code).
