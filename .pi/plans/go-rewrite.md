...
**LANDED 2026-08-06 — see `.pi/plans/go-rewrite-p1.md` (state: committed, 8 oracles green).** All ten packages plus `internal/hook` landed. Work item 8's "five TS-correct deltas" were not portable this session (contract artifacts were session-local to the planning run) — every guard, the ledger, and the classifier port bash bug-for-bug instead, since the A/B harness is byte-identical-or-red; revisit at P4. Two pre-existing bugs found and fixed while landing: a `comm` locale bug in `core/checkpoint.sh` and `core/restructure.sh` (both sorted operands with `LC_ALL=C` but ran `comm` under the ambient locale, spuriously blocking checkpoints); and a confirmed (not hypothetical) ownership-tracking gap where files written only by background `task` subagents were absent from the session ledger, recovered by re-writing them through a direct tool call.

**CORRECTION 2026-08-06 (validation pass).** This note previously claimed "real bash-vs-Go differential proof (policy: 54 vectors; comments: 167-file sweep; canonjson: pinned-jq golden vectors + randomized differential; bashglob: bash-verified table)". Two of those four claims did not hold, and the wording overstated the rest — P1 was faithful on well-formed input and diverged on malformed input. Measured: the shipped oracles exercised 70.6% of the 2359 ported statements, leaving 693 dark, and every defect found lived in that third. Re-measured against the pinned jq-1.7.1, `internal/canonjson` diverged on 134 of 739 corpus documents (all number formatting, in construction mode as well as round-trip); against real bash `case`, `internal/bashglob` diverged on 26/5072 pattern-name pairs under `LC_ALL=C` and 21/5072 under `en_US.UTF-8` — a DIFFERENT set, because `test/ab-hooks-test.sh` pinned `LC_ALL=C`, the one locale where GNU grep's and RE2's classes agree.

**REMEDIATED 2026-08-07.** All ten confirmed divergence classes are closed. Eight were Go-side parity fixes (config `null` fail-open, `protected_paths` element coercion, payload NUL/trailing-newline stripping to bash `$()` semantics, the `MustCompile` panic on untrusted needles, the nil-baseline SIGSEGV, langmap/config fail-open, locale-aware `[[:space:]]`, and the canonjson and bashglob divergences above). The last two were bash bugs fixed on BOTH legs under the new A31 exception: `write_state` now refuses to persist an empty or non-object document (it was atomically truncating the session ledger to 1 byte and exiting 0), and a non-whole-number baseline metric now fails loudly instead of making bash's `[ -gt ]` error and silently disabling the comment gate for that file. The A/B matrix is now 90 scenarios run under both `C` and `en_US.UTF-8`: 85 green, 5 registered structural limits (three NUL-payload and two malformed-ledger scenarios where exit and state agree and only bash's own tool diagnostics differ — see A32), 0 failures. The `LC_ALL=C` monoculture that hid the locale class is gone.
...
**LANDED 2026-08-07 — see `.pi/plans/go-rewrite-p2.md` (state: committed, 12 oracles green).** `internal/receipt` owns recipe v2; `internal/enginepin` + a tracked, kit-authored `engine.json` + a doctor attestation link land the pin. Three architectural readings were made explicit and are binding for P3+: (a) "routed through the engine via shims" is a DELEGATION seam, not a rewrite — `core/receipt-lib.sh`'s public functions capture the engine's `receipt` verbs while their bodies survive as byte-frozen `_v1` rollback legs, so push-gate/gated-push/checkpoint call sites are untouched; (b) exit code 2 is reserved forever for `main.go`'s unknown-verb default, which is how a bash wrapper detects version skew and falls back — no other handler may return it; (c) the cross-engine window test asserts DISJOINT generations (`recipeVersion`), not agreement, because resolution 7 versions the recipe rather than byte-chasing it, so flipping `SUBSTRATE_ENGINE` costs exactly one gate run by design.

Four defects in the first draft were found by an adversarial pass and fixed before landing, each worth carrying forward: the v2 record framing used `0x1F`/`0x1E` separators over raw path bytes and was therefore FORGEABLE (a reviewer produced a fingerprint collision) — it is now netstring length-prefixed; the engine pin had no fixed point until `-trimpath -buildvcs=false` landed on every build path, because Go stamps vcs state and the absolute build path into the binary; the doctor-attestation acceptance oracle could never match because `bin/substrate` colours its output unconditionally; and `substrate_engine_bin` collapsed the resolver's rc 2 so a forced `SUBSTRATE_ENGINE=go` silently answered from bash, which would have made every future forced-go oracle vacuous. B5's H6 A31 exception was also completed on the bash leg (missing, non-directory AND empty engine trees now fail closed on both). The duplication ratchet blocked the landing and was paid down by factoring the four hashers and the two delegation wrappers — `dup_pct` finished below its pre-P2 baseline rather than being waived.
...
- A31 Bash-authority exception (standing rule, established 2026-08-07 by the A1/B1 remediation): the DEFAULT is unchanged — bash is authoritative, the port reproduces it bug-for-bug, and the A/B harness is byte-identical-or-red; "more correct than bash" is a FAILURE, not an improvement. The exception is narrow and fires only where bash DESTROYS DATA or SILENTLY DISABLES A GUARD. When it fires: fix BOTH legs in the same batch, land the A/B scenario with the fix, and record it here. Precedents: `write_state` persisted an empty document whenever an upstream jq pipeline failed, atomically truncating the session ledger to 1 byte while exiting 0; and a non-whole-number `comments:<file>` baseline metric made bash's `[ -gt ]` error out so the ratchet never blocked, switching the comment gate off for that file. NOT covered — these stay bug-for-bug: git's C-quoted non-ASCII ledger keys (`test/golden/ledger/git-invalid-utf8.json`), the shebang comment-exemption, protect-command's leading-whitespace bypass, and anything merely surprising, cosmetic, or "cleaner" in Go. Judgement call: a divergence is only in scope if you can name the data lost or the guard disabled.
- A32 Structural limits are a third category, distinct from both parity bugs and A31 exceptions: where bash leaks diagnostics from its own subprocesses that the engine has no subprocess to emit (bash's `command substitution: ignored null byte in input` warning naming a script and line; jq's `Cannot index string with ...` parse errors), the verdict — exit code, stdout, watched state — must match, but the stderr text is NEVER forged. These are registered in the known-divergence registry in `test/ab-hooks-test.sh`, which reports them as expected and FAILS LOUDLY if a registered scenario ever starts agreeing, so the list cannot rot into a permanent excuse. Registry entries carry the reason and whether they are a structural limit or a parked decision; a parked decision is a bug awaiting a ruling, not a licence to diverge.
