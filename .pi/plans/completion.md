...
### Tracking machinery
- [x] maintenance report vendored, scheduled, queue live :: test -x core/report.sh && grep -q 'schedule:' core/ci/github-report.yml && [ -n "${GH_TOKEN:-}" ] || export GH_TOKEN=$(gh auth token -u secondary-user 2>/dev/null); if [ -z "${GH_TOKEN:-}" ]; then [ -z "${CI:-}" ] || exit 1; exit 0; fi && [ "$(gh issue list --label substrate-report --state open --json number --jq length)" -ge 1 ]
