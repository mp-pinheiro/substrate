...
checkpoint. Never hoard finished work in an uncommitted `@` — a step you didn't checkpoint is
not a recoverable publication point. Committing is local; **pushing** is the explicit user action
that publishes to a remote.
...
Checkpoint resolves and finalizes the local publication bookmark after verification.
No generic bookmark-advance command is recovery for a blocked checkpoint.
...
Address review comments by adding a commit on top, then explicitly update the intended
feature bookmark with `jj bookmark set my-feature -r @-` and push it. Because pushed
bookmarks are immutable (config above), you won't accidentally rewrite them.
...
| Publication bookmark | checkpoint receipt's `publicationBookmark` |
